...
- `--schedule-header-date-size` (`15px`), `--schedule-header-title-size` (`15px`), `--schedule-header-btn-size` (`32px`) and `--schedule-header-btn-icon-size` (`19px`) — the bar above the Schema calendar: the day headers, the week with its date range, and the round controls for moving between periods and switching between day, week and month. All of it is chrome around a grid and the app draws it at full size. Sizes rather than a scale on the bar, so its height stays the app's and the grid below does not move.
