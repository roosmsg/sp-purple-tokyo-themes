...
`shades-of-purple.css` carries the dark palette shown above in both slots.

The task detail panel, as a column of cards, in each light palette:

| Licht (`paper-purple.css`) | Tokyo Light (`minimal-tokyo.css`) |
| --- | --- |
...
And the Dashboard in Shades of Purple, with the sidebar collapsed to its icon rail:
...
Super Productivity paints the global wallpaper on every project and tag without a picture of its own, and has no "none" setting for a single one. To leave one context on the flat page colour, set its background image (project or tag → theme, per light/dark slot) to `https://` or `null`: the theme reads that placeholder as no wallpaper and hides the image, its veil and the floor beneath it.