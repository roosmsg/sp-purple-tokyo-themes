...
- `--break-reset-scale` (`0.75`), `--break-reset-opacity` (`0.55`) and `--break-reset-hover-ink` (the accent) — the reset button on the "Zonder pauze" reading, which the app brings in at full size and full strength when that reading is hovered. It arrives smaller and part-transparent instead, and goes to full strength and takes the accent when the pointer is on it — the same accent the play control carries, since this is the other button on that screen that starts a clock over.
