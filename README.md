...
| Action bar | In both the horizontal and experimental vertical layout, every button except Play and Add task sits at 25% opacity. They come up together when the pointer is on one of them — not when it is merely somewhere in the strip — and when the strip holds keyboard focus. The lift waits a moment first, so sweeping across the strip on the way somewhere else does not set it off; letting go is immediate. |
...
| Detail panel clearance | Starts the task detail panel below Electron's native window controls, which it otherwise opens underneath. The height is the same title-bar safe area the window-controls strip is drawn at, so the two cannot drift; `--detail-panel-top-gap` tunes it. |
...
- `--side-nav-scale`, `--nav-footer-scale`, `--action-bar-scale` — the navigation scale in three places: the left rail (`1.05`), the search/timeline/settings group at the foot of that rail relative to the rail itself (`1`, matching the other rail buttons), and the right action strip (`0.85`). `--action-bar-idle-opacity` (`0.25`) dims every action-bar button except Play and Add task by 75% while the bar is idle, in both desktop layouts. `--action-bar-undim-delay` (`250ms`) is how long the pointer must rest on one before the group comes up. `--play-button-size` (`46px`) sets the time-tracking button apart from the vertical strip scale; the strip widens to fit it.
