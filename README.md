...
| Action bar | In both the horizontal and experimental vertical layout, every button except Play and Add task sits at 25% opacity. They come up together when the pointer is on one of them — not when it is merely somewhere in the strip — and when the strip holds keyboard focus. The lift waits half a second first, so sweeping across the strip on the way somewhere else never sets it off; letting go is immediate. |
...
- `--side-nav-scale`, `--nav-footer-scale`, `--action-bar-scale` — the navigation scale in three places: the left rail (`1.05`), the search/timeline/settings group at the foot of that rail relative to the rail itself (`1`, matching the other rail buttons), and the right action strip (`0.85`). `--action-bar-idle-opacity` (`0.25`) dims every action-bar button except Play and Add task by 75% while the bar is idle, in both desktop layouts. `--action-bar-undim-delay` (`500ms`) is how long the pointer must be on the group before it comes up. `--play-button-size` (`46px`) sets the time-tracking button apart from the vertical strip scale; the strip widens to fit it. `--side-nav-icon-gap` (`15px`) is the space between a rail icon and its label; the app leaves 20px and this theme was collecting a further 12px from a rule written for context menus, which the rail matches because a nav link carries `mat-menu-item`. A plugin row keeps the app's own spacing: its icon is drawn differently and sits differently in the row.
