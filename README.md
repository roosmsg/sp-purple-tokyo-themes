...
- `--task-icon-size` — how large an icon is drawn in the task list, the view around it and the task detail panel, where an icon sits beside 14px text rather than beside a label in a rail or a menu. `20px`, against the 24px box it sits in, which does not change — so the icons step down without a row moving. Unlike `--icon-scale` this one is not about the plugin: it is stated as a font-size, which is the figure a glyph is drawn at, and the plugin draws to the same figure. Both builds therefore come out the same size, and those containers hand `--icon-scale` back to `1` rather than taking a second reduction off a size the theme already chose.
