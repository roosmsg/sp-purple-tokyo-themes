...
| Right panel clearance | Starts the task detail panel and the project notes below Electron's native window controls, which they otherwise open underneath. The height is the same title-bar safe area the window-controls strip is drawn at, so the two cannot drift; `--detail-panel-top-gap` tunes it. |
