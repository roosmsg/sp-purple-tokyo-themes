...
| Square hover states | Squares the hover highlight on the left menu rows only. Everywhere else Material's own shape stands: icon buttons stay circular, and list and option rows keep their radius. |
| Tab selection | Marks the selected tab with a straight line instead of a shape. The Settings page rounds a tab's top corners and fills the selected one, which reads as half a rounded rectangle sitting on the header rule; the radius and the fill are dropped, the hover layer follows the tab, and the active indicator is squared at its ends. |
