...
- `--break-reset-scale` (`0.75`) and `--break-reset-opacity` (`0.55`) — the reset button on the "Zonder pauze" reading, which the app brings in at full size and full strength when that reading is hovered. It arrives smaller and part-transparent instead, and goes to full strength when the pointer is on it.
