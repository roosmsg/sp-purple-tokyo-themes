...
| Action bar | In both the horizontal and experimental vertical layout, every button except Play and Add task sits at 25% opacity. They come up together when the pointer is on one of them — not when it is merely somewhere in the strip — and when the strip holds keyboard focus. They hold for a moment after the pointer leaves before fading back, so crossing a gap between two buttons does not drop them in between. |
...
- `--side-nav-scale`, `--nav-footer-scale`, `--action-bar-scale` — the navigation scale in three places: the left rail (`1.05`), the search/timeline/settings group at the foot of that rail relative to the rail itself (`1`, matching the other rail buttons), and the right action strip (`0.85`). `--action-bar-idle-opacity` (`0.25`) dims every action-bar button except Play and Add task by 75% while the bar is idle, in both desktop layouts. `--action-bar-fade-out-delay` (`400ms`) is how long they hold before fading back. `--play-button-size` (`46px`) sets the time-tracking button apart from the vertical strip scale; the strip widens to fit it.
